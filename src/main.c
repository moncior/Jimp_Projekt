...
static void print_usage(const char *program_name) {
	fprintf(stderr,
			"Uzycie:\n"
			"  %s -i <wejscie.txt> -o <wyjscie.txt> -a <algorytm> -f <format>\n\n"
...
static int write_nodes_text(const Graph *g, const char *filename) {
	...
	if (file == NULL) {
		fprintf(stderr, "Błąd: Nie można otworzyć pliku z danymi\n");
		return 0;
	}
...
static int load_graph_from_text(Graph *g, const char *filename) {
	...
	while (fgets(line, sizeof(line), file) != NULL) {
		...
		if (matched != 4) {
			fclose(file);
			fprintf(stderr, "Błąd: Nieprawidłowy format danych w pliku\n");
			return 0;
		}
...
int main(int argc, char **argv) {
	if (argc != 9) {
		fprintf(stderr, "Błąd: Nieprawidłowe parametry wywołania.\n");
		print_usage(argv[0]);
		return 1;
	}
